...
function handleOnMessage(aiClient) {
    rtm.on('message', (event) => {
        aiClient.ask(event.text, (err, res) => {
            ...
            if(!res.intent) {
                return rtm.sendMessage('Sorry, I did not understand', conversationId, function messageSent(){
                });

            } else if( res.intent[0].value =='time' && res.location){
                return rtm.sendMessage(`I did yet know the time in ${res.location[0].value}`, conversationId, function messageSent(){
                });
            } else {
                console.log(res)
            // The argument is the event as shown in the reference docs.
            // For example, https://api.slack.com/events/user_typing
            rtm.sendMessage('Sorry, I did not understand', conversationId, function messageSent(){
            });
            }
        });
    })
